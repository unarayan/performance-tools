...
- **Video File**
    ...
    ```
    **_Note:_** Video files must be in the **sample-media** folder, so that the Docker container can access the files. You can provide your own video files or download a sample video file using the script [download_sample_videos.sh](https://github.com/intel-retail/performance-tools/blob/main/benchmark-scripts/download_sample_videos.sh).
